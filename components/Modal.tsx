...
const Modal: React.FC<ModalProps> = ({ campaign, onClose }) => {
  ...
  return (
    <div
      className={`fixed inset-0 bg-black/70 backdrop-blur-lg z-50 flex items-center justify-center p-4 ${isClosing ? 'modal-leave' : 'modal-enter'}`}
      onClick={handleClose}
      role="dialog"
      aria-modal="true"
    ...
    >
      <div
        className={`bg-dark-gray/70 backdrop-blur-xl border border-slate/20 rounded-lg shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col md:flex-row overflow-hidden ${isClosing ? 'modal-content-leave' : 'modal-content-enter'}`}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="w-full md:w-2/3 bg-black/50 flex items-center justify-center">
          ...
          <div className="flex-grow">
            ...
            <ul className="space-y-2">
              {campaign.metrics.map((metric, index) => (
                <li key={index} className="flex justify-between items-center text-sm bg-black/70 backdrop-blur-sm p-2.5 rounded-md">
